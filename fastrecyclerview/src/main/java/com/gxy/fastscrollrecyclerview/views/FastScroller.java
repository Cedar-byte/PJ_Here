...
package com.gxy.fastscrollrecyclerview.views;
...
import com.gxy.fastscrollrecyclerview.R;
import com.gxy.fastscrollrecyclerview.utils.Utils;

public class FastScroller {
    private static final int DEFAULT_AUTO_HIDE_DELAY = 1500;// 默认自动隐藏延迟时间

    private FastScrollRecyclerView mRecyclerView;
    private FastScrollPopup mPopup;

    private int mThumbHeight;// 滑标的长度
    // 滑标的宽度,
    private int mWidth,mWidthBg;
...
//        canvas.drawRect(mThumbPosition.x + mOffset.x, mThumbPosition.y + mOffset.y, mThumbPosition.x + mOffset.x + mWidth, mThumbPosition.y + mOffset.y + mThumbHeight, mThumb);
        // 画圆角矩形滑块
        RectF oval3 = new RectF(mThumbPosition.x + mOffset.x, mThumbPosition.y + mOffset.y, mThumbPosition.x + mOffset.x + mWidth, mThumbPosition.y + mOffset.y + mThumbHeight);// 设置个新的长方形
